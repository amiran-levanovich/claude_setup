...
6b. **Commit gate on non-Claude harnesses** — if this session is not running under Claude Code (no PreToolUse hooks: Codex, Gemini CLI, or another Agent Skills harness), the gate can't intercept `git commit` from the harness side. Offer to install it as a plain git hook instead: write `.git/hooks/pre-commit` as a two-line wrapper (`#!/bin/sh` + `exec "<path-to-kit>/dev-workflow/.claude/hooks/pre-commit-gate.sh" --git-hook`), `chmod +x` it, and — if a `pre-commit` hook already exists — show it and ask before touching it, never overwrite silently. Verify by running the wrapper once (exit 0 on a clean feature branch). The `--git-hook` mode skips the JSON parsing, so check 6's `jq`/`python3` requirement doesn't apply to it.
