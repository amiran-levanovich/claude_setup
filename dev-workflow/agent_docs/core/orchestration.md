...
## Harness capability fallbacks (Codex, Gemini CLI, other Agent Skills harnesses)

The kit was born on Claude Code, but the method is harness-neutral: the docs are plain markdown and the skills follow the open [Agent Skills](https://agentskills.io) standard, which Codex and Gemini CLI read natively. Four capabilities the docs reference are Claude-specific — each has a fallback, and the fallback is part of the method, not an excuse to skip the step:

| Capability                            | On Claude Code                            | On other harnesses                                                                                                                                                                                                                                                                                            |
| :------------------------------------- | :------------------------------------------ | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Commit gate                           | PreToolUse hook intercepts `git commit`   | A **plain git pre-commit hook** — `workflow-init` offers the install (a wrapper exec'ing `pre-commit-gate.sh --git-hook`). Same script, same checks, enforced by git itself.                                                                                                                                  |
| Sub-agents (`diff-reviewer`, fixers)  | `Task` dispatch with fresh context        | The agent `.md` files are self-contained prompts. For true fresh eyes, run one in a **second CLI instance** (`codex exec` / `gemini -p` with the agent file plus the diff range as the prompt). Without that, follow the agent spec **inline as a strict protocol** — weaker (no fresh context), so say so in the Review log. Never skip the review itself. |
| `AskUserQuestion`                     | Native option-picker tool                 | Present the same **numbered options in chat** and stop until the user picks. The rule being enforced is "never decide a judgment call silently" — the tool is just its cheapest form.                                                                                                                          |
| Skill activation                      | Skill tool + auto-trigger on description  | Codex and Gemini discover `.agents/skills/` automatically (implicit match on the skill description, or explicit `$`-mention / `activate_skill`). Wiring instructions: the README's install options and `AGENTS.md`.                                                                                            |
