...
Read `agent_docs/core/coding_workflow.md` and follow it. Locate it as follows: use `agent_docs/core/coding_workflow.md` in the project root if present (drop-in install); otherwise read `../../../agent_docs/core/coding_workflow.md` relative to this skill's directory (plugin install).

It is the single, **language-agnostic** source of truth for the day-to-day TDD lifecycle, branch/commit hygiene, and the pre-commit gate. The spine detects the project language by marker file (`Gemfile` → `ruby`, `pyproject.toml`/`setup.py` → `python`) and routes the concrete tool commands to the matching `agent_docs/<lang>/toolchain.md`. Read that toolchain file too once the language is known.
