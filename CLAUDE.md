...
- **Versioning**: bump the affected plugin's `version` in its `plugin.json` on a meaningful change (breaking renames → major; `dev-workflow` is at 3.x, `craft-workflow` at 1.x). Bump with a targeted line edit — a JSON load/dump round-trip reformats the manifest.
